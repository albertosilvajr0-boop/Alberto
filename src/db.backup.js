...
async function getUserSettings(username) {
  ...
  return {
    ...
    anthropicModel: s.anthropicModel || "claude-3-5-sonnet-latest"
  };
}
...
async function getDecryptedKeys(username) {
  ...
  const s = (u && u.settings) || {};
  return {
    openaiKey: s.openaiKey ? decrypt(s.openaiKey) : null,
    openaiModel: s.openaiModel || "gpt-4o-mini",
    anthropicKey: s.anthropicKey ? decrypt(s.anthropicKey) : null,
    anthropicModel: s.anthropicModel || "claude-3-5-sonnet-latest"
  };
}
